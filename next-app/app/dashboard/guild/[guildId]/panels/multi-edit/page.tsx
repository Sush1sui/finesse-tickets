...
export default function EditMultiPanelPage() {
    ...
    const fetchMultiPanel = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (data.multiPanel) {
            const mp = data.multiPanel;
            setFormData({
              channel: mp.channel || "",
              selectedPanels: mp.panels?.length >= 2 ? mp.panels : ["", ""],
              useDropdown: mp.dropdownConfig?.use || false,
              dropdownPlaceholder: mp.dropdownConfig?.placeholder || "",
              embedColor: mp.messageEmbedConfig?.color || "#5865F2",
              title: mp.messageEmbedConfig?.title || "",
              description: mp.messageEmbedConfig?.description || "",
              authorName: mp.messageEmbedConfig?.authorName || "",
              authorIconUrl: mp.messageEmbedConfig?.authorImgUrl || "",
              authorUrl: mp.messageEmbedConfig?.authorUrl || "",
              largeImgUrl: mp.messageEmbedConfig?.largeImgUrl || "",
              smallImgUrl: mp.messageEmbedConfig?.smallImgUrl || "",
              footerText: mp.messageEmbedConfig?.footerText || "",
              footerImgUrl: mp.messageEmbedConfig?.footerImgUrl || "",
            });
          }
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      success("Multi-panel updated successfully!");
      setTimeout(() => {
        router.push(`/dashboard/guild/${guildId}/panels`);
      }, 1000);
    } catch (err) {
      console.error("Error updating multi-panel:", err);
      error("Failed to update multi-panel. Please try again.");
    } finally {
      setSaving(false);
    }
  };
        ...
        <div style={styles.main}>
          <div style={styles.card}>
            <div style={styles.header}>
              ...
              <h1 style={styles.title}>Edit Multi-Panel</h1>
            </div>
