					...
					{navItems.map((item) => {
						...
						return (
							<Link
								key={href}
								href={href}
								className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all duration-200 group ${isActive
									? "bg-[#FF5A36]/10 text-white border border-[#FF5A36]/20"
									: "text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800/40"
									}`}
							>
								<Icon
									className={`h-4 w-4 shrink-0 ${isActive ? "text-[#FF5A36]" : "group-hover:text-zinc-300"
										}`}
								/>
								<div>
					...
					<div className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-zinc-900/40 border border-zinc-800/40">
						{user.image ? (
							<Image
								...
								width={28}
								height={28}
								className="rounded-full ring-1 ring-zinc-700/60 shrink-0"
							/>
						) : (
